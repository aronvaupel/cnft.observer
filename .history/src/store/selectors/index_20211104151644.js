...
export const chartsFilter = createStructuredSelector({
    type: (state) => state.filters.type,
    options: (state) => state.filters.options,
    series: (state) => state.filters.series,
    width: (state) => state.filters.width,
    height: (state) => state.filters.height
});

export const chartItems = createSelector(chartsFilter, chartsBreakdownState, ( filters, charts ) => {
    let { data } = charts;
    const { type, options, series,} = filters;
    ...
    if(type) {
        data = data.filter( chart => type.has(chart.type));
    }
    if(options) {
        data = data.filter( chart => options.has(chart.options));
    }
    if(series) {
        data = data.filter( chart => series.has(chart.series));
    }
